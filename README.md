...
- **掉落通知**：在副本掉落预览页或战斗结算页一键订阅任意物品，命中时弹出物品图标提醒
...
- **🚪 自动后退**：基于攻击次数 / 掉落 / 召唤 / 技能配置自动后退
- **🔔 掉落通知**：从当前页的掉落预览或战斗结算页选取要监听的物品；命中时弹窗
- **⚙️ 配置管理**：导入导出配置数据
- **❌ 取消**：关闭菜单
...
- **🗑️ 删除标签**：删除选中的标签
- **🚪 自动后退【全局】**：等同于主菜单的自动后退入口
- **🔔 掉落通知【全局】**：等同于主菜单的掉落通知入口
- **❌ 取消**：关闭菜单
...
- **游戏增强**：可自定义订阅的掉落通知，扫描结算页 `.prt-item-list` 内的物品 `data-key`
...
- `sb_drop_subscriptions`: 掉落通知订阅列表（JSON 数组，每项含 `itemId / kind / iconUrl`）
...
### v2.1.0 (2026-05-19)
- 🔔 **掉落通知重写**：移除硬编码的 FFJ / 沙漏 / 大巴角 开关，改为用户自选订阅
  - 从副本掉落预览页（`.prt-drop-item-list`）或战斗结算页（`.prt-item-list`）勾选物品
  - 命中时弹出物品图标，多个同时掉落并排显示
  - 长按菜单加入「🔔 掉落通知【全局】」入口
- 🐛 **修复检测误报**：把 `data-key` 查询限定到 `.prt-item-list` 容器内，避免下一战预览 / 角色面板等区域撞 id 触发假阳性
- 🐛 **修复 PC 触屏长按**：触屏 PC 上 touch + mouse 双事件曾互相覆盖 long-press timer，导致短按变长按；改用时间戳 + clearTimeout 修复
- ⚙️ **掉落订阅纳入导入导出**
